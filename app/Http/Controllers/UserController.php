...
class UserController extends Controller
{
    public function getUserHomePage()
    {
        ...
        $buku = Buku::all();
        return view('user/userhome',['username' => $username,'user' => $user, "buku" => $buku]);
    }
    ...
    public function searchNonBuku(Request $request)
    {
        ...
        if($nonbuku)
        {
            $updatedHtml = View::make('user/searchresult2', compact('nonbuku'))->render();
            return response()->json(['html' => $updatedHtml]);
        }
    }
    ...
    {
        $nonbuku = NonBuku::find($request->id_nonbuku);
        $param["nonbuku"] = $nonbuku;
        return view('user/detailnonbuku',$param);
    }
